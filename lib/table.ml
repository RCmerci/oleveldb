...
module Make (Internal_cmp : Cmp.S) = struct
  module Block_iter = Block.Iter_make (Internal_cmp)
  ...
  let read_meta file footer =
    ...
    >>= fun metaindex_block ->
    let metaindex_iter = Block_iter.create metaindex_block in
    let policy_name =
      "filter." ^ Filter_block.BloomFilterBlock.policy_name
      |> Slice.from_string
    in
    let m =
      let open Block_iter in
      seek policy_name
      >>= fun b ->
      match b with
    ...
    in
    let filter_block =
      match Block_iter.run m metaindex_iter with
      | Ok (r, _) -> r
      | Error e -> Error e
    in
  ...
  let create file =
    ...
    if size < footer_encoded_length then
      ...
      if readn < footer_encoded_length_int then
        Error "incomplete sstable footer"
      else
  ...
  let block_reader t index_value =
    ...
    | None ->
        ...
        return block )
    >>= fun block ->
    let iter = Block_iter.create block in
    return iter
  ...
  module Two_level_iter = Two_level_iterator.Make (A) (Block_iter) (Block_iter)

  let create_iter t =
    let index_iter = Block_iter.create t.index_block in
  ...
  let get t k =
    ...
    in
    let open Option in
    ( match Block_iter.run m index_iter with
    | Ok (index_value, _) -> Some index_value
    | Error _ -> None )
    >>= fun index_value ->
    block_reader t index_value
    >>= fun block_iter ->
    match Block_iter.run m block_iter with
    | Ok (block_value, _) -> Some block_value
    | Error _ -> None

end

module Table_test = Make (Cmp.ByteWiseComparator)
